		...
		if clientType == "ai_agent" {
			...
			if spiffeID == nil || *spiffeID == "" {
				...
				if err != nil {
					...
					spireResp, err := dc.spireService.CreateAgentEntry(&services.SpireCreateAgentEntryRequest{
			...
			if resolvedSpiffeID != "" {
				...
				if len(delegatedPerms) == 0 {
					...
					jwtResp, jwtErr := dc.spireService.IssueDelegatedJWTSVID(&services.SpireIssueJWTSVIDRequest{
						TenantID:     tenantID.String(),
