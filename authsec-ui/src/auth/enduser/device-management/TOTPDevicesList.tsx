/**
 * TOTP Devices List Component - Compact Professional UI
 */
...
interface TOTPDevicesListProps {
  token: string;
  onBack: () => void;
}
...
export const TOTPDevicesList: React.FC<TOTPDevicesListProps> = ({ token, onBack }) => {
  const [fetchDevices, { data, isLoading, error, isFetching }] = useLazyGetTOTPDevicesQuery();
  const [deleteDevice, { isLoading: isDeleting }] = useDeleteTOTPDeviceMutation();
  ...
  const handleDelete = async (deviceId: string, deviceName: string) => {
    if (
      !confirm(`Are you sure you want to delete "${deviceName}"? This action cannot be undone.`)
    ) {
      return;
    }
    ...
    >
      ...
      {!isLoading && !error && devices.length === 0 && (
        <div className="text-center py-6">
          <IconDeviceMobile className="w-8 h-8 mx-auto text-slate-300 dark:text-slate-600 mb-2" />
          <p className="text-xs text-slate-500 dark:text-slate-400">No devices registered</p>
        </div>
      )}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  {device.is_primary && (
                    <IconStarFilled className="w-3 h-3 text-amber-500 flex-shrink-0" />
                  )}
                  {device.is_active && (
                ...
                <p className="text-[10px] text-slate-400 truncate">
                  {getDeviceTypeLabel(device.device_type)} · {formatDate(device.created_at)}
                </p>
              </div>
              <button
...
export default TOTPDevicesList;
